...
@FacesConverter("levelFourRequestableConverter3")

public class LevelFourRequestableConverter3 implements Converter {
	...
	public Object getAsObject(FacesContext fc, UIComponent uic, String value) {
		if (value != null && value.trim().length() > 0) {
			try {
				...
				List<LevelFourRequestableRow> levelFourList = reportView.getTmalignReportChoices();
				
				for (LevelFourRequestableRow row : levelFourList){
        			if (row.getQueryDisplayName().equals(value.replaceAll("\\n",""))){
        				return row;
        			}
        		}
	...
	public String getAsString(FacesContext fc, UIComponent uic, Object object) {
		if (object != null) {
			return String.valueOf(((LevelFourRequestableRow) object).getQueryDisplayName());
		} else {
